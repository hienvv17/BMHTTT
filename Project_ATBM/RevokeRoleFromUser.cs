...
namespace Project_BMHTTT
{
    public partial class RevokeRoleFromUser : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            commandCheckToTruong.CommandText = "pr_revoke_role_from_user";
            commandCheckToTruong.CommandType = CommandType.StoredProcedure;

           commandCheckToTruong.Parameters.Add(new OracleParameter("pi_user", OracleDbType.NVarchar2)).Value = roleCombobox.Text.Trim().ToUpper();// pwd.Text.Trim();
            commandCheckToTruong.Parameters.Add(new OracleParameter("pi_role", OracleDbType.NVarchar2)).Value = usernameCombobox.Text.Trim().ToUpper();// pwd.Text.Trim();
            try
            {
